...
{
	for (size_t i = 0; i < MEDIA_MAX_TRACK_NUM; i++)
	{
...
bool MediaSession::addRtpSink(MediaSession::TrackId trackId, std::shared_ptr<RtpSink> rtpsink)
{
	Track *track;
...
bool MediaSession::addRtpInstance(MediaSession::TrackId trackId, std::shared_ptr< RtpInstance>rtpInstance)
{
	Track *track;
	track = getTrack(trackId);
...
bool MediaSession::removeRtpInstance(std::shared_ptr<RtpInstance> rtpInstance)
{
	for (size_t i = 0; i < MEDIA_MAX_TRACK_NUM; i++)
	{
		...
		std::list<std::shared_ptr<RtpInstance>>::iterator iter = std::find(mTracks[i].mRtpInstances.begin(), mTracks[i].mRtpInstances.end(), rtpInstance);

		if (iter == mTracks[i].mRtpInstances.end())
		{
			continue;
		}
		mTracks[i].mRtpInstances.erase(iter);
		return true;
	}
	return false;
}
...
{
	MediaSession* media = static_cast<MediaSession*>(arg1);
	MediaSession::Track* track= static_cast<MediaSession::Track*>(arg2);
	if (!media && !track)
	{
		media->sendPacket(track, rtpPacket);
	}
}

void MediaSession::sendPacket(MediaSession::Track* track, RtpPacket* rtpPacket)
{
	std::list<std::shared_ptr<RtpInstance>>::iterator it;
...
bool MediaSession::startMulticast()
{
	...
	mMulticastRtpInstances[TrackId0] = std::make_shared<RtpInstance>(rtpSockfd1, 0, mMulticastAddr, rtpPort1);
	mMulticastRtpInstances[TrackId1] = std::make_shared<RtpInstance>(rtpSockfd2, 0, mMulticastAddr, rtpPort2);
	mMulticastRtcpInstances[TrackId0] = std::make_shared<RtcpInstance>(rtcpSockfd1, 0, mMulticastAddr, rtcpPort1);
	mMulticastRtcpInstances[TrackId1] = std::make_shared<RtcpInstance>(rtcpSockfd2, 0, mMulticastAddr, rtcpPort2);
